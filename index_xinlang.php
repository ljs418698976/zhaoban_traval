...
define("BAIDUMAPURL", "http://zhaoban.sinaapp.com/baidumap.php");
...
//exit;
//var_dump($city_weather_arr);
//$wechatObj = new getweather($city_weather_arr);
//$hh=array();
//$hh = $wechatObj->check_pub('上海');
//print_r($hh);
    ...
    public function responseMsg()
    {
            ...
            $msgType = $MsgType;
                if('text' == $msgType){
                          ...
                          if(!empty($data)){
                            ...
                            $contentStr = "点击进入<a href=\"http://zhaoban.sinaapp.com/admin/?uid={$fromUsername}\">后台登录页面</a>";
                                  $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                                    ...
                                    $musicTpl = '';
                                    foreach ($this->config_arr['推荐歌曲'] as $music_choose){
                                                  $MusicUrl = $music_choose['musicurl'];
                                                  $MusicUrl = trim($MusicUrl);
                                                  $contentStr = $music_choose['name'];
                                                  $description = $music_choose['description'];
                                                  $musicTpl = "<xml>
                                                                           <ToUserName><![CDATA[%s]]></ToUserName>
                                                                           <FromUserName><![CDATA[%s]]></FromUserName>
                                                                           <CreateTime>%s</CreateTime>
                                  ...
                                  $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                                    ...
                                    if(isset($info_add[0]) && isset($info_add[1])){
                                           if('姓名' == $info_add[1]){
                                                  ...
                                                  $data = $mysql->getData( $sql );
                                                  if(empty($data)){
                                                                  $sql = "INSERT  INTO `user` ( `uid` , `name` ) VALUES ( '"  . $fromUsername . "' , '" . $mysql->escape( $info_add[2] ) . "') ";
                                                          $mysql->runSql( $sql );
                                                                           $contentStr = "创建".$info_add[1].'成功';

                                                          }else{
                                                                  $sql = "UPDATE `app_zhaoban`.`user` SET `name` = '".$mysql->escape( $info_add[2] )."' WHERE `user`.`uid` ='{$fromUsername}'";
                                                                  $mysql->runSql( $sql );
                                                  ...
                                                  $data = $mysql->getData( $sql );
                                                  if(empty($data)){
                                                                  $sql = "INSERT  INTO `user` ( `uid` , `sex` ) VALUES ( '"  . $fromUsername . "' , '" . $mysql->escape( $info_add[2] ) . "') ";
                                                          $mysql->runSql( $sql );
                                                                           $contentStr = "创建".$info_add[1].'成功';

                                                          }else{
                                                                  $sql = "UPDATE `app_zhaoban`.`user` SET `sex` = '".$mysql->escape( $info_add[2] )."' WHERE `user`.`uid` ='{$fromUsername}'";
                                                                  $mysql->runSql( $sql );
                                                                           $contentStr = "修改".$info_add[1].'成功';
                                                  ...
                                                  $data = $mysql->getData( $sql );
                                                  if(empty($data)){
                                                                  $sql = "INSERT  INTO `user` ( `uid` , `weixin` ) VALUES ( '"  . $fromUsername . "' , '" . $mysql->escape( $info_add[2] ) . "') ";
                                                          $mysql->runSql( $sql );
                                                                           $contentStr = "创建".$info_add[1].'成功';

                                                          }else{
                                                                  $sql = "UPDATE `app_zhaoban`.`user` SET `weixin` = '".$mysql->escape( $info_add[2] )."' WHERE `user`.`uid` ='{$fromUsername}'";
                                                                  $mysql->runSql( $sql );
                                                                           $contentStr = "修改".$info_add[1].'成功';
                                                  ...
                                                  if(empty($data[0]['name']) || empty($data[0]['sex']) || empty($data[0]['weixin'])){
                                                                  ...
                                                                  $sql = "INSERT  INTO `traval` ( `uid` , `traval_content` , `create_time` ) VALUES ( '"  . $fromUsername . "' , '" . $mysql->escape( $info_add[2] ) . "' , NOW() ) ";
                                                                          $mysql->runSql( $sql );
                                                                          $contentStr = "创建".$info_add[1].'成功';
                                                          }
                    ...
                    //foreach ($this->config_arr['固定内容'] as $key => $fixed){
                    //					else if( $key== $keyword ){
                    //					$contentStr = $fixed;
                    //     $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                    //					}
                    //}
                                  	...
                                  	$contentStr = '';
                                  //$this->biaoqing[] = $keyword;
                                  //$fp = fopen('biaoqing.txt','a+');
                                  //fwrite($fp,"hjghj\r\n");
                                  //fclose($fp);
                             
                                  //$contentStr = "创建biaoqing成功";
                                  //$biaoqing_add = explode('#', $keyword);
                                  //$contentStr = $biaoqing_add[1];
                              //if(false !== strpos($keyword, ' ')){
                              //$biaoqing_arr = explode(' ', $keyword);
                                     //$sql = "INSERT  INTO biaoqing ( `expression_name` , `expression_code` ) VALUES (  '" .  $biaoqing_arr[1]  . "' ,'" .  $biaoqing_arr[0]  . "' ) ";
                                     //if($mysql->runSql( $sql )) {
                                     //$contentStr = "创建".$biaoqing_arr[1].'成功';
                                     // }else{
                                     // $contentStr = "创建".$biaoqing_arr[1].'失败';
                                     //}
                               // }
                                   $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                                  //$sql = "INSERT  INTO `biaoqing` ( `expression_name`, `expression_code` ) VALUES ( '{$biaoqing_add[0]}','"  . $contentStr . "') ";
                                  //$mysql->runSql( $sql );
                            }
                    }else{
